...
interface AirtimeReceiptScreenProps {
  receiptData: AirtimeReceiptData;
  onClose: () => void;
}
...
export const AirtimeReceiptScreen: React.FC<AirtimeReceiptScreenProps> = ({
  receiptData,
  onClose,
}) => {
  ...
  const formatDate = (dateString: string) => {
    try {
      ...
      if (dateString.includes('T')) {
        ...
        // Format like "September 8th, 2025"
  ...
  const getNetworkImage = (network?: string | null) => {
    if (!network) return require('@/assets/images/logo.png'); // Default to SureTopUp logo if no network
    const networkKey = network.toLowerCase();
    return NETWORK_IMAGES[networkKey as keyof typeof NETWORK_IMAGES] || require('@/assets/images/logo.png');
  };

  const getNetworkColor = (network?: string | null) => {
    if (!network) return '#00A651'; // Default to green background if no network
    const networkKey = network.toLowerCase();
    return NETWORK_COLORS[networkKey as keyof typeof NETWORK_COLORS] || '#00A651';
  };
  ...
  const isUserReceipt = selectedReceiptType === 'user';
      ...
      >
        ...
        >
          <View style={styles.receipt}>
            ...
            <View style={styles.transactionDetails}>
              ...
              {/* Balance Changes - Only show for user receipts */}
              {isUserReceipt && receiptData.oldBalance && receiptData.newBalance && (
                <>
                  <View style={styles.detailRow}>
        ...
        <View style={styles.actionButtons}>
          <TouchableOpacity
            ...
            onPress={() => setSelectedReceiptType(selectedReceiptType === 'user' ? 'seller' : 'user')}
          >
            <Ionicons 
              name={selectedReceiptType === 'user' ? 'business' : 'person'} 
              size={20} 
              color="white" 
            />
            <Text style={styles.actionButtonText}>
              {selectedReceiptType === 'user' ? 'Seller Receipt' : 'User Receipt'}
            </Text>
          </TouchableOpacity>
...
export default AirtimeReceiptScreen;
