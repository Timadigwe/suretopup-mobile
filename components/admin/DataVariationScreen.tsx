...
interface DataVariationScreenProps {
  onBack: () => void;
}

export const DataVariationScreen: React.FC<DataVariationScreenProps> = ({ onBack }) => {
  const { colors } = useTheme();
  const { triggerHapticFeedback } = useMobileFeatures();
  const { safeAreaTop, safeAreaBottom } = useSafeArea();
  
  const [variations, setVariations] = useState<DataVariation[]>([]);
  const [filteredVariations, setFilteredVariations] = useState<DataVariation[]>([]);
  ...
  const fetchDataVariations = async () => {
    setIsLoading(true);
    try {
      const response = await apiService.getDataVariations();
      ...
      Alert.alert('Error', 'Failed to fetch data variations');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const filterVariations = () => {
    ...
    if (searchQuery.trim()) {
      filtered = filtered.filter(item => 
        item.data_plan.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handleUpdateVariation = async (id: number, percentage: number) => {
    setIsUpdating(true);
    try {
      const response = await apiService.updateDataVariation(id, percentage);
      if (response.success) {
        ...
        fetchDataVariations(); // Refresh data
      } else {
  ...
  const openEditModal = (variation: DataVariation) => {
  ...
  const handleBulkUpdate = async () => {
    ...
    setIsUpdating(true);
    try {
      const response = await apiService.bulkUpdateDataVariations({
  ...
  const renderVariationItem = ({ item }: { item: DataVariation }) => (
    <TouchableOpacity
      style={[
    ...
    >
      <View style={styles.variationHeader}>
        <View style={styles.variationInfo}>
          ...
          <Text style={[styles.dataPlan, { color: colors.mutedForeground }]}>{item.data_plan}</Text>
        </View>
    ...
    >
      ...
      <View style={[styles.header, { backgroundColor: colors.card, paddingTop: safeAreaTop + 20 }]}>
        <View style={styles.headerContent}>
          ...
          <Text style={[styles.headerTitle, { color: colors.text }]}>Data Variations</Text>
          <TouchableOpacity
            onPress={fetchDataVariations}
      ...
      >
        ...
        <View style={styles.variationsList}>
          <Text style={[styles.listTitle, { color: colors.text }]}>
            {filteredVariations.length} Data Plan{filteredVariations.length !== 1 ? 's' : ''}
          </Text>
          ...
          <View style={[styles.modalContent, { backgroundColor: colors.card }]}>
            <View style={styles.modalHeader}>
              ...
              <Text style={[styles.modalTitle, { color: colors.text }]}>
                {successData.type === 'single' ? 'Variation Updated!' : 'Bulk Update Complete!'}
              </Text>
            </View>
...
export default DataVariationScreen;
